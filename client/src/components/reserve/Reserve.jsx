...
const Reserve = ({ setOpen, hotelId }) => {
    ...
    //To get number of days in a range of dates
    ...
    //Get all selected dates (from dates state)
    const allDates = getDatesInRange(dates[0].startDate, dates[0].endDate);

    //Check if room is available so we can disable room number checkbox
    const isAvailable = (roomNumber) => {
        const isFound = roomNumber.unavailableDates.some((date) =>
            allDates.includes(new Date(date).getTime())
        );

        return !isFound; //If it returns true it means room is not available
    };
    ...
    const handleSelect = (e) => {
        ...
        setSelectedRooms(checked
            ...
            : selectedRooms.filter((item) => item !== value) //Remove room
        );
    };
